...
dat_combined <- dat_unique_id %>% 
  left_join(dat_age, by = "unique_id") %>% 
  left_join(dat_gender, by = "unique_id")

dat_combined %>%  count(unique_id) %>% 
  arrange(desc(n))
dat_unique_id
dat_age
dat_gender
...
data_amp_complete <- data_amp_tidied %>% 
  count(unique_id, name = "n_trials") %>% 
  filter(n_trials == "72")

data_amp_tidied_complete <- data_amp_tidied %>% 
  semi_join(data_amp_complete, by = "unique_id")
...
```{r}
data_amp_tidied_complete %>%  count(rt) %>% arrange(rt)

test_fast_trial <- data_amp_tidied_complete %>% 
  mutate(fast_trial_test = ifelse(rt < 100, 1, 0)) %>% 
  group_by(unique_id) %>% 
  summarise(percent_fast_trial_test = mean(fast_trial_test)*100)
